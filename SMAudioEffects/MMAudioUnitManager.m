...
@property (copy, nonatomic) audioCallBack mAudioBack;

@end

@implementation MMAudioUnitManager

static MMAudioUnitManager *sharedInstace = nil;
+ (instancetype)sharedInstance {
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        sharedInstace = [[MMAudioUnitManager alloc] init];
...
static OSStatus PlayCallback(void *inRefCon,
    ...
    if (receivePcmData.length >= buffLen) {
        NSData *data = [[NSData alloc] initWithBytesNoCopy:receivePcmData.mutableBytes length:buffLen];
        ...
        MMAudioUnitManager *manager = (__bridge MMAudioUnitManager *)inRefCon;
        if(manager.mAudioBack){
            manager.mAudioBack(ioData);
        }
